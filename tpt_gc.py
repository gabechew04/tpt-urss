...
def calc_conditional_prob_current(forward_comms, backward_comms, P):
    """
    ...
    N = len(forward_comms) - 1
    num_states = len(P[0])
    
    prob_current_matrix = np.zeros((N, num_states, num_states)) # Initialise the probability current matrix
    
    for n in range(N):
        prob_current_matrix[n] =  backward_comms[n].reshape((num_states,1)) * P[n] * forward_comms[n+1]
...
def calc_rate(prob_current, A, B):
    ...
    prob_current : array
        An array of size (N, num_states, num_states) consisting of the probability current matrices of the chain for each time.
    A : list of ints
        The elements in the reactant set.
    B : list of ints
    ...
    -------
    effec_current : array
        An array of size (N+1, num_states) that is non-zero only in the indices corresponding to elements in the reactant and target set.
    ...
    rates = np.zeros((N+1, num_states)) 
    
    rates[0, A] = prob_current[0, A].sum() # Directly set the rate at A at time 0
    rates[N, B] = prob_current[N - 1, :, B].sum() # Directly set the rate at B at time N.
    
    for n in range(1, N):
        rates[n, A] = prob_current[n, A].sum() # Sum of outflow from A
        rates[n, B] = prob_current[n - 1, :, B].sum() # Sum of inflow into B from previous timestep 
